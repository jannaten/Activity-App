...
/** Empty state (no demo activities — easier to reason about). */
const emptyState = {
  activities: [],
  activitiesActive: [],
  activitiesArchived: [],
...
describe('addActivity', () => {
  beforeEach(() => {
    // Pin "now" to 08:00:00 so we can craft a future time reliably
    vi.setSystemTime(new Date(2025, 0, 1, 8, 0, 0)); // 08:00:00
  });
  afterEach(() => {
    vi.useRealTimers();
  });
  ...
  it('does NOT add activity if the scheduled time is in the past', () => {
    // timeStr 07:00 is earlier than our frozen "now" of 08:00
...
describe('reorderActivities', () => {
  it('reorders activitiesActive and keeps archived at the end of activities', () => {
    ...
    // Archived should follow active in the full activities list
...
describe('decrementTime', () => {
  ...
  it('does not decrement time on archived (completed) activities', () => {
    ...
    const next = reducer(state, decrementTime());
    // Archived is not in activitiesActive so its timeSet is unchanged
