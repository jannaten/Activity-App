...
// Mock useNavigate so the component doesn't actually navigate in tests
...
/**
 * Returns a time string (HH:MM) that is guaranteed to be at least 1 hour in the future.
 * We cap at 22:30 to avoid crossing midnight during CI runs.
 */
function futuretime() {
  const d = new Date();
...
describe('CreateActivity', () => {
  ...
  it('shows success message and dispatches addActivity on valid submit', async () => {
    ...
    // Verify the activity was added to the Redux store
    const state = store.getState();
    expect(state.activities.activities.some((a) => a.name === 'Morning run')).toBe(true);
  });

  it('navigates to dashboard after the 1200ms delay post-submit', async () => {
    // Only this test needs fake timers to control the setTimeout in the component
    vi.useFakeTimers();
    ...
    try {
      ...
      // Advance past the 1200ms navigation timer
      await act(async () => {
